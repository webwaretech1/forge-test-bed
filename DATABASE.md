...
- `migrations/001_create_scores_table.sql` - Creates the main scores table with proper indexing and replay verification  
...
| `player_name` | VARCHAR(50) | NOT NULL | Player name (max 50 characters) |
| `game_slug` | VARCHAR(50) | NOT NULL | Game identifier |
| `score` | BIGINT | NOT NULL, >= 0 | Player's score |
...
- `idx_scores_leaderboard` - Unique composite index on `(game_slug, score DESC, timestamp ASC)` for efficient leaderboard queries with tie-breaking
...
- Player names are limited to 50 characters maximum
